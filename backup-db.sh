...
docker exec freshrss-db /bin/bash \
  ...
  | gzip -9 > /backups/freshrss_db_$(date "+%F-%H%M%S").sql.gzip
  
# docker exec invidious-db /bin/bash \
#   -c "export PGPASSWORD=$INVIDIOUS_DB_PASS \
#   && pg_dump -U $INVIDIOUS_DB_USER $INVIDIOUS_DB_NAME" \
#   | gzip -9 > /backups/invidious_db_$(date "+%F-%H%M%S").sql.gzip  


#Volumes
docker run --rm --volumes-from freshrss -v $(pwd):/backup ubuntu tar zcvf /backup/freshrss_config_$(date "+%F-%H%M%S").tar.gz /config
